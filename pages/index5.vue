<template>
      ...
      <div v-else>
        ...
        <div v-else>
          ...
          <section class="mb-5">
            ...
            <div class="row">
              <div v-for="track in recentlyPlayed.slice(0,6)" :key="track.played_at" class="col-12 mb-2">
          ...
          <!-- <section class="mb-5">
            ...
            <div class="row">
                <div
                v-for="playlist in madeForYouPlaylists.slice(0, 3)"
                :key="playlist.id"
                class="col-md-4 col-12 mb-3"
                >
                <div class="d-flex align-items-center p-2 rounded hover-bg-dark">
                    <img
                    :src="playlist.images[0]?.url || '/img/placeholder-album.png'"
                    class="me-3"
                    width="48"
                    height="48"
                    :alt="playlist.name"
                    ...
                    <div class="artist-name text-muted">By {{ playlist.owner.display_name }}</div>
                    </div>
                    <button
                ...
                <div v-if="madeForYouPlaylists.length === 0" class="col-12">
                <div class="alert alert-dark">
                    <p class="mb-0">No personalized playlists found. Explore more to get recommendations!</p>
                </div>
                </div>
            </div>
            </section> -->
  ...
  const { getMyTopArtists, getMyRecentlyPlayed, login, hasToken, isConnected, isInitialized, getMadeForYouPlaylists } = useSpotify()
  const { playTrack: playerPlayTrack, isPlayerReady } = useSpotifyPlayer()
  const { user, isAuthenticated } = useAuth()
  const isLoading = ref(true)
  const madeForYouPlaylists=ref<SpotifyPlaylist[]>([])
  ...
  const loadMusicData = async () => {
    ...
    try {
      ...
      const [artistsData, recentlyPlayedData,madeforyouData] = await Promise.allSettled([
      ...
      if (madeforyouData.status === 'fulfilled' && madeforyouData.value) {
  madeForYouPlaylists.value = madeforyouData.value as SpotifyPlaylist[]
}
